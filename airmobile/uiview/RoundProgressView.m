...
-(id)initWithCenter:(CGPoint)center radius:(CGFloat) radius aboveColos:(NSArray *)aboveColos belowColos:(NSArray *)belowColos start:(CGFloat)start end:(CGFloat)end clockwise:(BOOL)clockwise
{
    ...
    if (self)
    {
        ...
        UIImageView *bgImage = [CommonFunction imageView:@"home_circle.png" frame:CGRectMake(0, 0, radius*2, radius*2)];
        bgImage.center = self.center;
        [self addSubview:bgImage];
        
        [self configCircle:radius start:start end:end clockwise:clockwise];
...
-(void)configCircle: (CGFloat)radius start:(CGFloat)start end:(CGFloat)end clockwise:(BOOL)clockwise
{
    ...
    UIBezierPath * bez = [UIBezierPath bezierPathWithArcCenter:self.center radius:radius-lineWidth/2 startAngle:degreesToRadians(start) endAngle:degreesToRadians(end) clockwise:clockwise];//[UIBezierPath bezierPathWithRoundedRect:rect cornerRadius:radius];
    ...
    UIBezierPath * bez1 = [UIBezierPath bezierPathWithArcCenter:self.center radius:radius-lineWidth/2 startAngle:degreesToRadians(start) endAngle:degreesToRadians(end) clockwise:clockwise];
    ...
    [self.layer insertSublayer:self.cancelLaber below:self.abnormalLaber];
    
    
    UIBezierPath * bez2 = [UIBezierPath bezierPathWithArcCenter:self.center radius:radius-lineWidth/2 startAngle:degreesToRadians(start) endAngle:degreesToRadians(end) clockwise:clockwise];//[UIBezierPath bezierPathWithRoundedRect:rect cornerRadius:radius];
    ...
    [self.layer insertSublayer:self.normalLaber above:self.abnormalLaber];
    ...
    gradientLayer2.colors = @[(__bridge id)[CommonFunction colorFromHex:0XFFFE3838].CGColor,(__bridge id)[CommonFunction colorFromHex:0XFFFF6039].CGColor ];
    gradientLayer2.startPoint = CGPointMake(0,0.5);
    gradientLayer2.endPoint = CGPointMake(0.5,1);
    [self.layer insertSublayer:gradientLayer2 above:self.cancelLaber];
...
-(void)animationWithStrokeEnd:(CGFloat)strokeEnd withProgressType :(ProgreesType)type
{
    ...
    switch (type) {
        ...
        case ProgreesTypeAbnormal:
            [self.abnormalLaber pop_addAnimation:bAni forKey:@"AbnormoalAnimation"];
            break;
            
        case ProgreesTypeCancel:
            [self.cancelLaber pop_addAnimation:bAni forKey:@"AbnormoalAnimation"];
            break;
...
-(void)doAnimation
{
    [self animationWithStrokeEnd:_normalProportion withProgressType:ProgreesTypeNormal];
