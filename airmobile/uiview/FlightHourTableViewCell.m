...
-(instancetype) initWithStyle: (UITableViewCellStyle)style psnArea: (PassengerAreaModel *)psnArea
{
    self = [super init];
    if(self){
        [self.contentView addSubview:[CommonFunction addLabelFrame:CGRectMake(0, 0, kScreenWidth/2-20, viewHeight(self.contentView)) text:psnArea.region font:35/2 textAlignment:(NSTextAlignmentLeft) colorFromHex:0xFF000000]];
        [self.contentView addSubview:[CommonFunction addLabelFrame:CGRectMake(kScreenWidth/2, 0, kScreenWidth/2-40, viewHeight(self.contentView)) text:[NSString stringWithFormat:@"%i",(int)psnArea.count] font:35/2 textAlignment:(NSTextAlignmentRight) colorFromHex:0xFF000000]];
...
-(instancetype) initWithStyle: (UITableViewCellStyle)style reuseIdentifier: (NSString *)identifier flightHour: (FlightHourModel *)flightHour
{
    self = [super init];
    if(self){
        UILabel *timeLabel = [[UILabel alloc]initWithFrame:CGRectMake(px2(32), 0, (kScreenWidth-2*px2(32))/2, viewHeight(self.contentView))];
        timeLabel.text = flightHour.hour;
        timeLabel.font = [UIFont fontWithName:@"PingFang SC" size:px2(36)];
        [self.contentView addSubview:timeLabel];


        UILabel *countLabel = [[UILabel alloc]initWithFrame:CGRectMake(px2(32)+(kScreenWidth-2*px2(32))/2, 0, (kScreenWidth-2*px2(32))/2, viewHeight(self.contentView))];
        ...
        UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(px2(32), viewHeight(self.contentView)-0.5, kScreenWidth-2*px2(32), 0.5 )];
        lineView.backgroundColor = [UIColor grayColor];
        lineView.alpha = 0.5;
        [self.contentView addSubview:lineView];
    }
