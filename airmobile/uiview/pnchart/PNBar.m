...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        barView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        [self addSubview:barView];
        
        _chartLine              = [CAShapeLayer layer];
        _chartLine.lineCap      = kCALineCapButt;
        _chartLine.fillColor    = [[UIColor whiteColor] CGColor];
        _chartLine.lineWidth    = barView.frame.size.width;
...
-(void)setBarRadius:(CGFloat)barRadius
{
    ...
    barView.layer.cornerRadius = _barRadius;
}
...
- (void)setGrade:(float)grade
{
    ...
    barView.frame = CGRectMake(0, startPosY, self.frame.size.width, self.frame.size.height-startPosY);

    UIBezierPath *progressline = [UIBezierPath bezierPath];

    [progressline moveToPoint:CGPointMake(self.frame.size.width / 2.0, barView.frame.size.height)];
    ...
    if (_barColor) {
        ...
        _chartLine.strokeColor = [[CommonFunction colorFromHex:0xFFB4D8FA] CGColor];
    }
