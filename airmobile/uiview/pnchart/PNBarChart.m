...
- (void)setupDefaultValues
{
    ...
    _yLabelSum           = 4;
    _labelMarginTop      = 0;
    _chartMarginLeft     = 10.0;
    _chartMarginRight    = 10.0;
        ...
        for (int index = 0; index < _xLabels.count; index++) {
            ...
            if (labelAddCount == _xLabelSkip) {
                ...
                label.center = CGPointMake(labelXPosition,
                                           self.frame.size.height - kXLabelHeight - _chartMarginBottom + label.frame.size.height /2.0 + _labelMarginTop);
                labelAddCount = 0;
...
- (void)updateBar
{
    ...
    for (NSNumber *valueString in _yValues) {
        ...
        if (_bars.count == _yValues.count) {
           ...
           CGFloat y = self.frame.size.height - chartCavanHeight - kXLabelHeight - _chartMarginBottom;
           bar = [[PNBar alloc] initWithFrame:CGRectMake(barXPosition, //Bar X position
                                                         y<0? y+ _chartMarginTop:y, //Bar Y position
                                                          barWidth, // Bar witdh
