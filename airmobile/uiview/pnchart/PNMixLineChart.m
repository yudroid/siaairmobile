...
//  airmobile
//
//  Created by xuesong on 16/12/22.
...
- (void)drawRect:(CGRect)rect {
    if (self.isShowCoordinateAxis) {
        ...
        if ([self.yUnit length]) {
            CGFloat height = [PNMixLineChart sizeOfString:self.yUnit withWidth:30.f font:font].height;
        ...
        if ([self.xUnit length]) {
            CGFloat height = [PNMixLineChart sizeOfString:self.xUnit withWidth:30.f font:font].height;
...
- (UIView *)getLegendWithMaxWidth:(CGFloat)mWidth {
    ...
    CGFloat singleRowHeight = [PNMixLineChart sizeOfString:@"Test"
                                              withWidth:MAXFLOAT
    ...
    for (PNLineChartData *pdata in self.chartData) {
        /* Expected label size*/
        CGSize labelsize = [PNMixLineChart sizeOfString:pdata.dataTitle
